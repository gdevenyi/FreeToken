...
def _select_extend_tiles(
    ...
    V follow the cache, so a 1-byte fp8 cache fits a tile a 16-bit one cannot.
    """
    budget = smem_optin * 0.8  # headroom for scores/acc/alignment/triton scratch

    if head_dim <= 256:
        # (64, 64) is only reachable by a 1-byte cache where a 16-bit one needs 64x32.
        ladder = [(128, 64)] + ([(64, 64)] if kv_bytes < 2 else []) + [(64, 32), (32, 32), (16, 16)]
    elif head_dim <= 384:
        ladder = [(32, 64), (32, 32), (16, 16)]
    else:
        ladder = [(32, 64), (16, 16)]
    if not smem_optin:
        return ladder[ladder.index((64, 32)):] if (64, 32) in ladder else ladder[1:]
    first = next(
        (i for i, (m, n) in enumerate(ladder) if (m * 2 + 2 * n * kv_bytes) * block_d <= budget),
        len(ladder) - 1,
    )
    return ladder[first:]
...
def _select_decode_tile(
    block_d: int, block_dv: int, smem_optin: int, kv_bytes: int = 2
) -> tuple[int, int]:
    ...
    for block_n, stages in ((32, 2), (32, 1), (16, 1)):
        if stages * block_n * (block_d + block_dv) * kv_bytes <= budget:
...
def decode_paged_attention(
    ...
    block_n, num_stages = _select_decode_tile(
        block_d, block_dv, _optin_smem_bytes(q.device.index), 2 if nvfp4 else k_cache.element_size()
    )
...
def extend_paged_attention(
    ...
    ``block_ends`` holds per query token the end of the multimodal span it sits in (0 for
    none), whose later keys the row also attends.
    """
    ...
    tiles = _select_extend_tiles(head_dim, block_d, _optin_smem_bytes(q.device.index), kv_tile_bytes)
